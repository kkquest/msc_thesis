...
\begin{abstract}
    ...
    Two methodological extensions are examined: an alternative preprocessing pipeline (combining exponential smoothing, rolling
    variance normalisation, and seasonal differencing), and the integration of the Expected Quantile Discrepancy (EQD) criterion
    for automated threshold selection. Their validity is assessed through a diagnostic framework of dependence and tail-model
    checks, applied to three Numenta Anomaly Benchmark datasets (CPU, Twitter, and Traffic), each exhibiting distinct forms of
    non-stationarity.

    Results show that preprocessing has complementary strengths but clear limits. A central finding is the importance of 
    preprocessing: EVT-based detectors only perform reliably when residual extremes come close to independence, a condition
...
this matters directly: a series of consecutive exceedances may reflect one genuine event rather than many, yet naively counting
them as independent can distort the fitted model. Addressing non-stationarity is therefore a central requirement for making EVT
...
but they also highlight the limitations of the approach. Preprocessing is deliberately simple, often limited to moving averages,
and this is not enough when non-stationarity arises from seasonality or time-varying variance. Thresholds are usually fixed by
...
and diagnostics. Chapter~\ref{sec:res} reports empirical results on three benchmark datasets. Chapter~\ref{sec:discussion} discusses implications, 
limitations, and directions for future work, followed by a plain-language conclusion.
...
is not met, the procedure can even deteriorate performance. This finding underlines that advances in thresholding are valuable,
but their effectiveness ultimately hinges on how well preprocessing aligns the data with EVT’s theoretical requirements.

The empirical results show that the three benchmark datasets exhibit distinct types of non-stationarity--mean shifts in CPU,
...
clearer gains, further reducing dependence among exceedances while maintaining an excellent GPD fit, and in this setting EQD
...
and EQD then tended to worsen calibration. Taken together, these results emphasise that preprocessing has complementary 
roles but also clear limits. POT-based detection remains viable when preprocessing succeeds in producing residual extremes 
that are close to independent, yet the theory falters when clustering persists, as in the Twitter case.
...
in classical EVT, it is well recognised that detrending or de-seasonalising a series can change the behaviour of the extremes,
sometimes reducing the information available for tail inference if applied too strongly \citep{EastoeTawn2009}. What is novel
...
methods, reflecting that all approaches suffered from low precision driven by many false positives. Recognising these 
limitations is important, since over-interpreting small-sample metrics risks attributing significance where there is none.
...
this mapping for the enhanced preprocessing pipeline, but the resulting curves were highly sensitive to small fluctuations
...
Readers should therefore interpret the reported thresholds as operating in that working domain, rather than expecting a direct
...
With the right preprocessing, they can provide reliable alarms with clear error rates. But they are not a silver bullet: 
when data remain dependent or anomalies are not extreme, EVT methods can miss important signals. The challenge ahead is 
