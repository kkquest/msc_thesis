...
class dSPOT(SPOT):
    ...
    def initialize(self, level: float = 0.98):
        ...
        # actual normal window of last `depth` init points
        window: np.ndarray = self._init_data[-self._depth:]

        # prepare lists
        thresholds = []
        alarms     = []
        means      = []    # <-- collect the local moving‐average at each step
        ...
        for i, datum in enumerate(self._data):
            ...
            status = self._ev.run(datum - mean, self._num, with_alarm=with_alarm)
            ...
            # record the threshold back on the original scale
            thresholds.append(self._ev.extreme_quantile + mean)

        # now pad the init‐period so `means` lines up with the full series
        init_means = [np.nan] * len(self._init_data)
        all_means  = init_means + means
