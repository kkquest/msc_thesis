...
class SPOTBase(ABC):
    ...
    def plot(self, run_results: dict, with_alarm: bool = True, title: str = None, label: str = None):
        """
        Plot residuals (or raw series if residuals missing) and dynamic thresholds.
        Alarms are marked in red.
        """
        ...
        if residuals is not None:
            plt.plot(x, residuals, label="Residuals", color="steelblue")
        else:
            raw_series = np.concatenate([self._init_data, self._data])
            plt.plot(x, raw_series, label="Raw series", color="steelblue")

        # Plot threshold
        if thresholds:
            start_idx = len(self._init_data)
            threshold_x = np.arange(start_idx, start_idx + len(thresholds))
            plt.plot(threshold_x, thresholds, '--', label="Threshold", color="orange")

        # Plot alarms
        if with_alarm and alarms:
            alarm_x = [len(self._init_data) + a for a in alarms]
            alarm_y = [residuals[idx] if residuals is not None else self._data[a] for a, idx in zip(alarms, alarm_x)]
            plt.scatter(alarm_x, alarm_y, color="red", label="Alarms")

        plt.xlabel("Time Index")
        plt.ylabel(label or ("Z-score" if residuals is not None else "Raw Value"))
        plt.title(title or "SPOT Residuals and Dynamic Thresholds")
        plt.legend()
        plt.tight_layout()
...
class OnlinePreprocessor:
    ...
    - Seasonal differencing (fixed lag)
    ...
    def __init__(self, P=288, alpha=0.05):
        ...
        self.T_hist = deque(maxlen=P)  # EWMA trend history (for seasonal diff)
        self.x_hist = deque(maxlen=P)  # Raw history (for seasonal diff)
        self.z_window = deque(maxlen=P)  # Rolling window for std estimation
        self.std_eps = 1e-8  # Small epsilon to avoid division by zero

    def update(self, x_t):
        """
        Process a new data point `x_t` and return the normalized residual z-score.
        """
        # EWMA trend update
        ...
        # Append to histories
        self.T_hist.append(T_t)
        self.x_hist.append(x_t)

        # Seasonal differencing (if we have enough history)
        if len(self.x_hist) == self.P:
            S_t = self.x_hist[0] - self.T_hist[0]
        else:
            S_t = 0.0  # Not enough history yet

        # Residual
        resid = x_t - T_t - S_t
        self.z_window.append(resid)

        # Rolling standard deviation (if enough residuals)
        if len(self.z_window) >= 5:
            std = np.std(self.z_window, ddof=1)
        else:
            std = None

        # Compute z-score (normalized residual)
        if std is None or std < self.std_eps:
            z_t = None  # Can't compute z-score yet
        else:
            z_t = resid / std
...
class ModifiedSPOT(SPOT):
    """
    SPOT + custom preprocessing: EWMA + seasonal differencing + std norm.
    Designed for stationary series (preprocessing enforces stationarity).
    """
    ...
    def initialize(self, level: float = 0.98):
        """
        Initialize EVT model using raw initial data with full preprocessing applied.
        """
        data = []
    ...
    def run(self, with_alarm=True):
        """
        Run SPOT with online preprocessing.
        """
        thresholds = []
        alarms = []
        self._residuals = []
